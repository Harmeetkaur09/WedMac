...
export default function BlogPage() {
  const blogPosts = [
    {
      ...
      description: "At enim sapien ante nullam placerat felis pellentesque nam, orci justo odio.",
  ...
  const popularTopics = ["Life Style", "Advanture","Recipe",  "Technology", "Education", "Design"]
...
<div className="grid grid-cols-1  md:grid-cols-2 gap-x-4 gap-y-8 place-items-center">
  ...
  <Card className="w-[350px] overflow-hidden hover:shadow-lg transition-shadow cursor-pointer">
    <div className="relative">
      ...
      />
      <Badge className={`absolute top-4 right-4 p-1 font-inter rounded-none ${post.categoryColor} text-white`}>
        {post.category}
      </Badge>
    </div>
    <CardContent className="p-4">
      <h3 className="text-xl font-inter font-[500] mb-3 line-clamp-2">{post.title}</h3>
      <p className="text-[#6c757d] text-sm font-inter mb-4 line-clamp-2">{post.description}</p>
      <div className="flex items-center text-sm text-gray-500">
        <span className="font-inter">By <b>{post.author}</b></span>
