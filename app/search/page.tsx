...
useEffect(() => {
  ...
  const filteredArtists = artists.filter((artist) => {
    ...
    const types = artist.makeup_types.map((t) => t.toLowerCase());

    const matchesType = !typeFilter || types.includes(typeFilter); // both sides are lowercase

    const matchesState = !stateFilter || loc.includes(stateFilter); // lowercase vs lowercase

    const matchesCity = !cityFilter || loc.includes(cityFilter);
  ...
  useEffect(() => {
    ...
    fetch("https://wedmac-services.onrender.com/api/artists/cards/")
      ...
      .then((data: ArtistCard[]) => {
        setArtists(data); // ← load the whole array here
      })
    ...
    // fetch makeup_types
    fetch(
      "https://wedmac-services.onrender.com/api/admin/master/list/?type=makeup_types"
    )
      .then((res) => res.json())
    ...
    // products (services)
    fetch(
      "https://wedmac-services.onrender.com/api/admin/master/list/?type=services"
    )
      .then((res) => res.json())
      .then((data: { id: number; name: string; description: string }[]) => {
              ...
              <div className="mb-6">
                ...
                >
                  <option
                    value=""
                    disabled
                    className="text-sm text-[#303A4280]"
                  >
                    Enter Your Location
                  </option>
                ...
                <div className="space-y-2 h-full ">
                  {products.map((prod) => (
                    <div key={prod} className="flex items-center space-x-2">
                      ...
                      <Label htmlFor={`prod-${prod}`} className="text-sm">
                        {prod}
                      </Label>
                    </div>
                  ))}
